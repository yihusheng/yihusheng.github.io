...
(function renderDrawer() {
  ...
  container.innerHTML = filtered.map(function(item) {
    return '<a class="drawer-row" href="' + item.href + '">' +
      '<div class="drawer-row-icon"><span class="material-symbols-rounded">' + item.icon + '</span></div>' +
      '<div class="drawer-row-info"><div class="drawer-row-label">' + item.label + '</div>' +
      '<div class="drawer-row-hint">' + item.href + '</div></div>' +
      '<div class="drawer-row-arrow"><span class="material-symbols-rounded">chevron_right</span></div></a>';
  }).join('');
})();
...
var coverInfoArea = document.getElementById('coverInfoArea');
var lyricsBody = document.getElementById('lyricsBody');
var lyricsHeaderTitle = document.getElementById('lyricsHeaderTitle');
var lyricsHeaderArtist = document.getElementById('lyricsHeaderArtist');
var lyricsBackBtn = document.getElementById('lyricsBackBtn');
...
function renderLyricsBody() {
  ...
  lyricsHeaderTitle.textContent = document.getElementById('mainTitle').textContent;
  lyricsHeaderArtist.textContent = document.getElementById('mainArtist').textContent;

  if (currentLyrics.length === 0) {
    lyricsBody.innerHTML = '<div class="lyrics-empty">暂无歌词</div>';
    return;
  }

  var html = currentLyrics.map(function(line, i) {
    return '<div class="lyrics-line" data-time="' + line.time + '" data-index="' + i + '">' + line.text + '</div>';
  }).join('');

  lyricsBody.innerHTML = html;

  // 点击歌词跳转
  var lineEls = lyricsBody.querySelectorAll('.lyrics-line');
...
function updateLyrics(currentTime) {
  if (currentLyrics.length === 0 || !isLyricsMode) return;
  ...
  if (newIndex !== currentLyricIndex) {
    ...
    if (newIndex >= 0 && lineEls[newIndex]) {
      lineEls[newIndex].scrollIntoView({behavior: 'smooth', block: 'center'});
    }
  }
}
...
function showCoverMode() {
  isLyricsMode = false;
  coverInfoArea.classList.remove('show-lyrics');
}

function showLyricsMode() {
  isLyricsMode = true;
  renderLyricsBody();
  coverInfoArea.classList.add('show-lyrics');
}
...
lyricsBackBtn.addEventListener('click', function() {
  showCoverMode();
});
...
audioSlider.addEventListener('input', function() {
  ...
  activeTrack.style.width = p + '%';
  inactiveTrack.style.left = p + '%';
  inactiveTrack.style.width = (100 - p) + '%';
...
// ==================== 天气 ====================
var weatherCodeMap = {
  0:{d:"晴朗",i:"wb_sunny"},1:{d:"多云",i:"partly_cloudy_day"},2:{d:"少云",i:"partly_cloudy_day"},3:{d:"阴天",i:"cloud"},
  45:{d:"雾",i:"foggy"},48:{d:"冻雾",i:"foggy"},51:{d:"毛毛雨",i:"rainy"},53:{d:"中毛毛雨",i:"rainy"},55:{d:"大毛毛雨",i:"rainy"},
  61:{d:"小雨",i:"rainy"},63:{d:"中雨",i:"rainy"},65:{d:"大雨",i:"thunderstorm"},66:{d:"冻雨",i:"severe_cold"},67:{d:"冻雨",i:"severe_cold"},
  71:{d:"小雪",i:"ac_unit"},73:{d:"中雪",i:"ac_unit"},75:{d:"大雪",i:"severe_cold"},77:{d:"雪粒",i:"severe_cold"},
  80:{d:"阵雨",i:"rainy"},81:{d:"阵雨",i:"rainy"},82:{d:"暴雨",i:"thunderstorm"},85:{d:"阵雪",i:"ac_unit"},86:{d:"阵雪",i:"ac_unit"},
  95:{d:"雷暴",i:"thunderstorm"},96:{d:"雷暴",i:"thunderstorm"},99:{d:"强雷暴",i:"thunderstorm"}
};
...
async function fetchWeather() {
  ...
  try {
    if (!locName) {
      ...
      var ld = await l.json();
      locName = ld.city || ld.locality || ld.principalSubdivision || lat.toFixed(2) + ', ' + lon.toFixed(2);
    }
    ...
    var d = await wr.json();
    var c = d.current_weather || d.current, da = d.daily, t = Math.round(c.temperature_2m), w = weatherCodeMap[c.weather_code] || {d: '未知', i: 'thermostat'};
